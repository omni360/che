    ...
    public EditorMultiPartStackViewImpl(PartStackUIResources resources) {
        ...
        partViewContainer = new AcceptsOneWidget() {
            ...
            public void setWidget(IsWidget widget) {
                ...
                int size = width/(widgets.size() + 1);
                width = size > 0 ? size : width;

                for (IsWidget child : widgets) {
                    contentPanel.setWidgetSize(child.asWidget(), width);
                }
                widgets.add(widget);
                contentPanel.addWest(widget, width);
...
//        tabsPanel.add(tabItem.getView());
    ...
    @Override
    public void addPartStack(@NotNull PartStack partStack) {
